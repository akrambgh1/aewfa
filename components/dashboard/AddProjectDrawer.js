...
export default function AddProjectDrawer({
  ...
  const isTypingLocationRef = useRef(false);
  ...
  // Resolve short maps links via server API
  const handleMapsUrlChange = async (url) => {
    setMapsUrl(url);
    setError("");
    ...
    try {
      ...
      if (res.ok && data.lat && data.lng) {
        ...
        setError(data.error || "Impossible d'extraire la position depuis ce lien.");
      }
    } catch (err) {
      setError("Erreur lors de la lecture du lien Google Maps.");
    } finally {
      setIsGeocoding(false);
    }
  };
  ...
  useEffect(() => {
    if (!form.location.trim() || !open || locationMode === "link") return;
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data && data.length > 0) {
            isTypingLocationRef.current = true;
            setSelectedLocation({
  ...
  }, [form.location, open, locationMode]);
  ...
  function handleImages(event) {
    ...
    const totalExisting = existingImages.length + images.length;
    const remaining = Math.max(0, 10 - totalExisting);

    if (remaining === 0) {
      setError("Vous ne pouvez pas avoir plus de 10 photos.");
      event.target.value = "";
      return;
    }

    setImages((current) => [...current, ...imageFiles.slice(0, remaining)]);
    setError("");
    event.target.value = "";
  }
  ...
  async function getImageKitAuth() {
    ...
    if (!response.ok) {
      throw new Error("Impossible d'authentifier l'envoi des photos.");
    }
    ...
    const result = await upload({
      ...
      onProgress: (event) => {
        ...
        const overallProgress =
          ((index + currentFileProgress) / total) * 100;
  ...
  async function uploadImagesToImageKit() {
    ...
    for (let index = 0; index < images.length; index++) {
      const uploaded = await uploadImage(images[index], index, images.length);
  ...
  async function handleSubmit(event) {
    ...
    if (!form.location.trim()) {
      setError("Veuillez saisir l'emplacement du projet.");
      return;
    }

    if (!form.apartments || Number(form.apartments) < 0) {
      setError("Veuillez saisir le nombre d'appartements.");
      return;
    }

    if (!selectedLocation) {
      setError("Veuillez définir une position sur la carte ou via un lien.");
      return;
    }
    ...
    try {
      ...
      const newImages = await uploadImagesToImageKit();
      ...
      console.error("Impossible d'enregistrer le projet :", err);
      setError(
        err?.message ||
              ...
              <h2 className="font-serif text-4xl font-light leading-[0.9] sm:text-5xl">
                {isEditing ? (
                  <>
                    Modifier
                    <br />
                    <em className="italic text-[#c4956a]">le projet</em>
                  </>
                ) : (
                  <>
                    Ajouter
                    <br />
                    <em className="italic text-[#c4956a]">un projet</em>
                  </>
                )}
              </h2>
            </div>
          ...
          <div className="flex-1 space-y-8 overflow-y-auto px-6 py-8 sm:px-8">
            ...
            <Field
              label={
                <>
                  Description
                  <span className="ml-2 text-[#c4956a]">Optionnel</span>
                </>
              }
            >
              <textarea
            ...
            <Field
              label={
                <div className="flex items-center justify-between">
                  <span>Emplacement (Nom de la ville)</span>
                  {isGeocoding && (
            ...
            <div>
              ...
              {/* ALWAYS SHOW MAP AFTER GEOLOCATION */}
              <div className="mt-4">
              ...
              {selectedLocation && (
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <div className="border border-[#c4956a33] bg-[#ede0cc] p-3">
                    ...
                    <span className="mt-1 block text-xs font-mono font-bold text-[#1a1410]">
